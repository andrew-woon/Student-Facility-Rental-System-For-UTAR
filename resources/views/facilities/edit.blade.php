<x-app-layout>
    <x-slot name="header">
        {{ __('Edit Facility: ') . $facility->name }}
    </x-slot>
                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    <form action="{{ route('facilities.update', $facility) }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <x-input type="text" name="name" id="name" value="{{ old('name', $facility->name) }}" placeholder="Enter name" class="w-full"/>
                            @error('name')<p class="text-red-500 text-xs">{{ $message }}</p>@enderror
                        </div>

                        <div class="mb-4">
                            <x-label for="location" class="block text-sm font-medium text-gray-700">Location</x-label>
                            <x-input type="text" name="location" id="location" value="{{ old('location', $facility->location) }}" placeholder="Enter location" class="w-full"/>
                            @error('location')<p class="text-red-500 text-xs">{{ $message }}</p>@enderror
                        </div>

                        <div class="mb-4">
                            <x-label for="capacity" class="block text-sm font-medium text-gray-700">Description</x-label>
